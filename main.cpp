...
int64_t sml180, sml280, smluknwn;
double uknwn;
...
typedef struct {
  ...
  const char name[20];
  const unsigned int devide;
 } smlsearch;

// clang-format off
smlsearch entries[] = {
  {{ 0x77, 0x07, 0x01, 0x00, 0x10, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x1b, 0x52, 0x00 }, 14, "Leistung_gesamt", 1000},
  {{ 0x77, 0x07, 0x01, 0x00, 0x01, 0x08, 0x00, 0xff, 0x65, 0x00, 0x1c, 0x01, 0x04, 0x01, 0x62, 0x1e, 0x52, 0xff }, 18, "Bezug", 10000},  
  {{ 0x77, 0x07, 0x01, 0x00, 0x02, 0x08, 0x00, 0xff, 0x01, 0x01, 0x62, 0x1e, 0x52, 0xff }, 14, "Abgabe", 10000},  
  {{ 0x77, 0x07, 0x01, 0x00, 0x24, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x1b, 0x52, 0x00 }, 14, "Leistung_L1", 1000}, 
  {{ 0x77, 0x07, 0x01, 0x00, 0x38, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x1b, 0x52, 0x00 }, 14, "Leistung_L2", 1000},
  {{ 0x77, 0x07, 0x01, 0x00, 0x4c, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x1b, 0x52, 0x00 }, 14, "Leistung_L3", 1000}
//  {{ 0x77, 0x07, 0x01, 0x00, 0x20, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x23, 0x52, 0xff }, 14, "20_7_0", 1},
//  {{ 0x77, 0x07, 0x01, 0x00, 0x34, 0x07, 0x00, 0xff, 0x01, 0x01, 0x62, 0x23, 0x52, 0xff }, 14, "34_7_0", 1},
//  {{ 0x77, 0x07, 0x01, 0x00, 0x51, 0x07, 0x1a, 0xff, 0x01, 0x01, 0x62, 0x08, 0x52, 0xff }, 14, "51_7_1A", 1},
//  {{ 0x77, 0x07, 0x01, 0x00, 0x51, 0x07, 0x02, 0xff, 0x01, 0x01, 0x62, 0x08, 0x52, 0xff }, 14, "51_7 2", 1}
};
// clang-format on
int smllen = 6;
  ...
  for (n = 0; n < slen; n++){
    for (i = 0; i <= smlIndex; i++) {
      ...
      if (found){
        ...
        memcpy(&smluknwn,&invert,8);
        Serial.printf("%llu\n",smluknwn);
        uknwn = double(smluknwn/double(entries[n].devide));
        Serial.printf("%.3f\n",uknwn);
        ...
        strcpy(path, clientId);
        strcat(path, topic);
        strcat(path,entries[n].name);
        sprintf(buffer,"%.3f",uknwn);
